...
def import_index_factor():
    ...
    data_index_month_logr = data_index_month_logr.dropna()
    return data_index_month_logr[index_factor_colname],data_index_month_logr[benchmark_name],data_index_month_logr[factor_colname]
...
def query_alpha_fund_type_data(book):
    fund_file_name =path_to_data_folder()+'FOF数据/fund_i_data_July.xlsx'
    data_book_i = pd.read_excel(fund_file_name,skiprows=[0,2,3,4],sheet_name =book,index_col=0,parse_dates=True)
    data_fund=data_book_i
    data_fund=data_fund.resample("M").last()
